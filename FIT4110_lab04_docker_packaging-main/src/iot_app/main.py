...
@app.get(
    "/sensor-events/recent", dependencies=[Depends(verify_bearer_token)]
)
...
@app.get(
    "/sensor-events/{event_id}",
    response_model=IoTEvent,
    dependencies=[Depends(verify_bearer_token)],
)
def get_sensor_event(event_id: str) -> IoTEvent:
    for event in EVENTS:
        if event["eventId"] == event_id:
            return IoTEvent(**event)
    raise HTTPException(
